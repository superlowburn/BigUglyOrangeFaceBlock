...
Classification, descriptions, policy, and reveal state stay on the device. Goggles has no analytics or server and sends no page text, image, or usage event to an extension-owned service. Site rules, description preferences, and the default new-site mode are the only persisted product state. Reveal state lasts only for the current document. Page descriptions are inserted as text, never as HTML, and page-generated synthetic clicks cannot reveal media. A deliberately revealed YouTube or Vimeo frame navigates to that provider, as the original page intended, with autoplay disabled.
