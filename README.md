...
5. Select `/Users/REDACTED/Projects/Fader/dist`.
...
The manifest requests `declarativeNetRequestWithHostAccess` plus host access only for `www.youtube.com`, `www.youtube-nocookie.com`, and `player.vimeo.com`. Those permissions are used solely to redirect recognized embed paths before network and authorize one selected embed. The `webNavigation` permission is used to remove temporary authorizations before top-level navigation; Eclipse Goggles ignores the event URL and does not keep browsing history. The extension does not inspect request bodies. The inert redirect document is web-accessible because Chrome requires that for a subframe redirect, but it contains no script or data.

Eclipse Goggles is a best-effort attention and consent tool, not a security boundary against a page script deliberately tampering with extension-managed DOM. A selected provider iframe's short-lived authorization URL is visible in its `src` attribute; a hostile script that intentionally copies that URL is outside this first release's threat model. Grants expire after 10 seconds and are also removed on re-protection, page teardown, tab close, top-level navigation, and extension-worker startup.
